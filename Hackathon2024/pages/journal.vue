<template>
  <v-container>
    <v-row class="justify-center">
      <v-col cols="12" md="8">
        <v-card>
          <v-card-title class="title-font"> Journal Entries </v-card-title>
          <v-card-text>
            <v-list>
              <v-list-item
    ...
    <v-dialog v-model="showEntryForm" max-width="600">
      <v-card>
        <v-card-title class="title-font"
          >{{ isEditing ? "Edit" : "Add" }} Journal Entry</v-card-title
        >
        <v-card-text>
          <v-text-field
            class="font"
        ...
        <v-card-actions>
          <v-btn class="font" color="primary" @click="saveEntry">{{
