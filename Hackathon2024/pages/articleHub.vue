  ...
  <v-container>
    ...
    <v-dialog v-model="showArticleDialog" max-width="800">
      <v-card>
        ...
        <v-card-actions>
          <v-btn class="font" text @click="showArticleDialog = false"
            >Close</v-btn
          >
        </v-card-actions>
      </v-card>
...
const articles = ref([
  ...
  },
  {
    ...
    date: "2024-10-28",
    summary:
      "Learn how to use Vuetify to style and build responsive applications.",
    content:
      "This article covers Vuetify basics, from setup to creating beautiful UIs...",
    category: "Healthy Life Choices",
  ...
  filteredArticles.value = articles.value.filter((article) => {
    return (
      ...
      (!selectedSubcategory.value ||
        article.subcategory === selectedSubcategory.value)
    );
  });
}
